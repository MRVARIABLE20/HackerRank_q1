...
from .routers import seed_files as seed_files_router
from .routers import upload as upload_router

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(name)s: %(message)s")
_settings = get_settings()

app = FastAPI(title="RAG Atlas", version="1.0.0")
app.add_middleware(
    CORSMiddleware,
...
def _startup() -> None:
    init_db()
    seed_if_empty()
    kb_docs_router.prewarm_suggestions()
...
app.include_router(seed_files_router.router)
